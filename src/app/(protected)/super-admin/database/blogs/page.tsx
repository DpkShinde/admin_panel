...
const EditDeleteBlog: React.FC = () => {
  ...
  const [selectedBlogEditorState, setSelectedBlogEditorState] =
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const contentRaw = JSON.stringify(
        convertToRaw(selectedBlogEditorState.getCurrentContent())
      );
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update blog: ${data?.error || response.status}`
        );
      }
      ...
      setBlogs(
        blogs.map((blog) =>
          blog.id === selectedBlog.id
            ? { ...selectedBlog, content: contentRaw }
            : blog
        )
      );
  ...
  const applyInlineStyle = (prefix: string, value: string) => {
    ...
    const currentStyles = selectedBlogEditorState
    ...
    setSelectedBlogEditorState(
      RichUtils.toggleInlineStyle(newState, styleToApply)
    );
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          onClick={() => router.push("/super-admin/database/blogs/createblogs")}
        >
          Add Blog
        </Button>
      </div>
      <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          {/* Adjusted DialogContent width for better responsiveness and slightly wider appearance on medium screens and up */}
          <DialogContent className="w-full max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto">
            <DialogHeader>
              <DialogTitle className="bg-white text-gray-600 border-gray-600">
                {isEdit ? "Edit Blog" : "Confirm Delete"}
              </DialogTitle>
            </DialogHeader>
            {/* Added padding (p-4 or p-6) to this div and ensured overflow-y for scrolling if content is too tall */}
            <div className="max-h-[calc(100vh-200px)] overflow-y-auto p-4 md:p-6 bg-white text-gray-600 border-gray-600">
              {isEdit && selectedBlog && selectedBlogEditorState ? (
                <div>
                  <div className="mb-4">
                    <label
                      htmlFor="title"
                      className="block font-small text-gray-700 mb-1" 
                    >
                      Blog Title:
                    </label>
                    <input
                  ...
                  {/* Added mb-4 for spacing below the entire editor block */}
                  <div className="border border-gray-300 rounded mb-4">
                    {/* Formatting toolbar: Increased padding (p-3), and added space-y-3 to stack sections vertically for clarity */}
                    <div className="bg-gray-100 p-3 border-b border-gray-300 space-y-3">
                      {/* Text formatting section: items-center for vertical alignment, increased gap-x-1.5 */}
                      ...
                      {/* Block type controls: items-center, increased gap-x-1.5 */}
                      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 pt-2 border-t border-gray-200"> {/* Added pt-2 and border-t for separation */}
                        <button
                          type="button"
                          className={`px-2.5 py-1.5 ${
                            isBlockType("header-one")
                              ? "bg-blue-200 text-blue-700"
                              : "bg-gray-200"
                          } hover:bg-gray-300 rounded text-xs`}
                      ...
                      {/* Font control section: items-center, increased gap-2, added min-w to selects */}
                      <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-gray-200">
                        <select
                          className="px-2 py-1.5 bg-gray-200 border-gray-300 rounded text-xs min-w-[130px] hover:bg-gray-300" 
                        ...
                        <select
                          className="px-2 py-1.5 bg-gray-200 border-gray-300 rounded text-xs min-w-[100px] hover:bg-gray-300" // Added min-w
                        ...
                        <select
                          className="px-2 py-1.5 bg-gray-200 border-gray-300 rounded text-xs min-w-[110px] hover:bg-gray-300" // Added min-w
                        ...
                        <select
                          className="px-2 py-1.5 bg-gray-200 border-gray-300 rounded text-xs min-w-[120px] hover:bg-gray-300" // Added min-w
                          onChange={(e) =>
                      ...
                      {/* Text alignment section: items-center, gap-x-1.5 */}
                      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 pt-2 border-t border-gray-200">
                        <button
                          type="button"
                          className={`px-2.5 py-1.5 ${
                            isBlockType("left") ? "bg-blue-200 text-blue-700" : "bg-gray-200"
                          } hover:bg-gray-300 rounded text-xs`}
                        ...
                        <button
                          ...
                          title="Align Center"
                        >
                          &harr; {/* Left Right Arrow HTML Entity */}
                        </button>
                        <button
                          type="button"
                          className={`px-2.5 py-1.5 ${
                            isBlockType("right") ? "bg-blue-200 text-blue-700" : "bg-gray-200"
                          } hover:bg-gray-300 rounded text-xs`}
                        ...
                        <button
                          ...
                          title="Justify"
                        >
                           {/* Using a simple justify icon or text */}
                    ...
                    {/* Editor area: Increased min-h and padding (p-4) */}
                    <div className="min-h-[300px] border-t border-gray-300 p-4 bg-white text-black draft-editor-wrapper">
                      {Editor && (
                        <Editor
                  ...
                  <div className="mt-4">
                    ...
                    <input
                      type="text"
                      className="w-full p-2 mt-1 border rounded" // Changed mt-2 to mt-1
