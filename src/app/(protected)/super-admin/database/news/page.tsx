...
const EditDeleteNews: React.FC = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to delete news: ${data?.error || response.status}`
        );
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update news: ${data?.error || response.status}`
        );
      }
    ...
    <div>
      ...
      <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="block font-small text-gray-700">
                {isEdit ? "Edit News" : "Confirm Delete"}
              </DialogTitle>
            </DialogHeader>
            <div>
              {isEdit ? (
                <div>
                  <label className="block font-small text-gray-700">
                    Title:
                  </label>
                  <input
                    type="text"
