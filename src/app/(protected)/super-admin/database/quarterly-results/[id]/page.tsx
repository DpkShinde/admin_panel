...
export default function EditEarningResult() {
  ...
  const restoreSelection = () => {
    if (selectionStateRef.current && editorRef.current?.contains(selectionStateRef.current.startNode)) {
      const selection = window.getSelection();
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
     setFormData(prev => {
        ...
        return {
            ...prev,
            [name]: name === 'company_id' ? (value ? parseInt(value, 10) : undefined) : value,
        };
     });
  };
  ...
  const execCommand = (command: string, value: string = '') => {
    if (editorRef.current) {
      ...
      const updatedContent = editorRef.current?.innerHTML || formData?.MainContent || '';
 ...
 const insertTable = () => {
   ...
   if (rows && cols) {
     ...
     if (numRows > 0 && numCols > 0) {
       let tableHTML = '<table style="border-collapse: collapse; width: 100%; margin-bottom: 16px;">';

       // Create header row if more than 1 row is requested
       if (numRows > 1) {
         tableHTML += '<thead><tr>';
         for (let j = 0; j < numCols; j++) {
           tableHTML += '<th style="border: 1px solid #ddd; padding: 8px; text-align: left; background-color: #f2f2f2;">Header ' + (j + 1) + '</th>';
         }
         tableHTML += '</tr></thead>';
       }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { company_id, title, MainContent: content, created_date } = submissionData; // Renamed MainContent to content for clarity in validation
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || `Error updating data: ${response.status}`);
      }
  ...
  if (loading) {
    return <div className="container mx-auto p-6 text-center text-gray-600">Loading...</div>;
  }

  if (error && !formData) { // Display fetch error if no data loaded
      return (
   ...
   if (!formData) { 
       return <div className="container mx-auto p-6 text-center text-gray-600">Earning result not found.</div>;
   }
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Edit Earning Result</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-4">
        {/* Company ID Input */}
        <div>
          <label htmlFor="company_id" className="block text-sm font-bold text-gray-700 mb-1">
            Company ID:
          </label>
          <input
            type="number"
            id="company_id"
            name="company_id"
            value={formData.company_id ?? ''} // Use ?? '' for number input when value might be 0 or undefined
        ...
        {/* Image URL Input */}
        <div>
          <label htmlFor="image_url" className="block text-sm font-bold text-gray-700 mb-1">
            Image URL:
          </label>
          <input
            type="text"
        ...
        <div>
          <label htmlFor="MainContent" className="block text-sm font-bold text-gray-700 mb-1">
            Main Content:
          </label>
          ...
          <div className="border border-gray-300 rounded-t-md bg-gray-100 p-1 flex flex-wrap gap-1">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M4 4a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1zm-1 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm1 4a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1zm-1 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd" />
              </svg>
            </button>
            <button
              type="button"
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M3 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clipRule="evenodd" transform="rotate(180 10 10)" />
              </svg>
            </button>
            ...
            <div className="flex items-center">
              <button
                type="button"
                onClick={() => {
                  const color = prompt('Enter text color (e.g. #ff0000, red):', '#000000');
              ...
              <button
                type="button"
                onClick={() => {
                  const color = prompt('Enter highlight color (e.g. #ffff00, yellow):', '#ffff00');
        ...
        {/* Created Date Input */}
        <div>
          <label htmlFor="created_date" className="block text-sm font-bold text-gray-700 mb-1">
            Created Date:
          </label>
          <input
            type="date"
