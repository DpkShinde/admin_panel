...
export default function EditStockResearch() {
  ...
  const [loadingResearchData, setLoadingResearchData] = useState<boolean>(true);
  ...
  const fetchResearchData = async () => {
    ...
    try {
      ...
      // Set form values
      if (researchData.stock) {
        setValue("stock.symbol", researchData.stock.symbol || "");
      }
  ...
  const onSubmit = async (data: FullStockResearchFormData) => {
    ...
    try {
      const response = await fetch(
        `/api/research-table/research-details/${researchId}`,
        {
      ...
      if (response.ok) {
        toast.success(
          result.message || "Stock research data updated successfully!"
        );
        ...
        if (response.status === 400 && result.errors) {
          ...
          toast.error(
            result.message || "An unknown error occurred during update."
          );
        }
      }
    } catch (error) {
      ...
      toast.error(
        "Failed to update stock research data. Please check your network connection."
      );
    } finally {
  ...
  if (loadingCompanies || loadingResearchData) {
    return (
  ...
  return (
    <div className="container mx-auto p-6 bg-white shadow-lg rounded-lg my-8">
      ...
      <h1 className="text-3xl font-extrabold text-center text-gray-800 mb-8">
        Edit Stock Research Information
      </h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex gap-4">
          <button
            type="button"
            onClick={() => router.back()}
            className="px-6 py-3 bg-gray-500 text-white font-semibold rounded-lg shadow-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 transition duration-150 ease-in-out"
          >
            Cancel
          </button>
          <button
            type="submit"
            className="flex-1 px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:bg-blue-400 disabled:cursor-not-allowed transition duration-150 ease-in-out"
            disabled={isSubmitting || !selectedCompanySymbol} // Disable if no company is selected
          >
            {isSubmitting ? "Updating..." : "Update Stock Research"}
          </button>
        </div>
      </form>
    </div>
