...
const getInputType = (key: string): string => {
  ...
  if (key.includes("url") || key.includes("website") || key.includes("tag")) return "url";

  const numericPatterns = [
    "_id", "year", "quarter", "_percent", "_rs", "amount", "value", "price", "margin",
    "eps", "roe", "roce", "nim", "car", "ttm_pe", "pb_ratio", "capital", "surplus",
    "interest", "deposits", "borrowings", "liabilities", "assets", "loans", "advances",
    "investments", "expenditure", "profit", "tax", "dii", "promoters", "public", "others",
    "performance", "revenue_cr", "change_percent", "gross_profit_percent", "net_profit_percent"
  ];

  if (numericPatterns.some((p) => key.toLowerCase().includes(p))) return "number";
  return "text";
};
...
export default function EditCompanyDataPage() {
  const [companyData, setCompanyData] = useState<CompanyFinancialResult | null>(null);
  ...
  if (!companyData) return <div className="p-6 text-center">Loading record...</div>;

  return (
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">Edit Company Financial Result</h1>
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-center">
          <Button type="submit" className="px-6 py-2 bg-green-600 text-white hover:bg-green-700">
            Update Record
          </Button>
        </div>
      </form>
