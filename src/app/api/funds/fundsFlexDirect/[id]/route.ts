...
export async function GET(
  ...
  try {
    const [rows]: any = await pool.query(
      `SELECT * FROM flexfundsDirect WHERE id = ?`,
      [etfId]
    );
...
export async function PUT(
  ...
  try {
    ...
    const updateQuery = `UPDATE flexfundsDirect SET fund_name = ?, nav = ?, aum_crore = ?, sip_amount = ?, 
    ...
    return NextResponse.json(
      {
        success: true,
        message: `flexfundsDirect fund updated successfully`,
      },
      { status: 200 }
    );
  } catch (error: any) {
...
export async function DELETE({ params }: { params: { id: string } }) {
  try {
    ...
    const [result]: any[] = await pool.query(
      `DELETE FROM flexfundsDirect WHERE id = ?`,
      [etfId]
    );
